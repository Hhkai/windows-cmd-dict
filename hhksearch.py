...
def colorful_print(raw):
    ...
    for line in lines:
        if line:
            if colorful:
                colorful = False
                winpycolor.colorprint(line + '\n', winpycolor.FOREGROUND_GREEN)
                continue
...
def search_database(word):
    ...
    res = curs.fetchall()
    if res:
        winpycolor.colorprint(word + ' 在数据库中存在', winpycolor.FOREGROUND_GREEN)
        print()
        winpycolor.colorprint('★ ' * res[0][1], winpycolor.FOREGROUND_RED)
        winpycolor.colorprint('☆ ' * (5 - res[0][1]), winpycolor.FOREGROUND_YELLOW)
        colorful_print(res[0][0])
    else:
        winpycolor.colorprint(word + ' 不在本地，从有道词典查询', winpycolor.FOREGROUND_RED)
        search_online(word)
        ...
        if add_in_db_pr and add_in_db_pr.isdigit():
            if(int(add_in_db_pr) >= 1 and int(add_in_db_pr) <= 5):
                ...
                winpycolor.colorprint('单词 {word} 已加入数据库中'.format(word=word), winpycolor.FOREGROUND_RED)
    curs.close()
    conn.close()
...
def add_word(word, default_pr):
    ...
    res = curs.fetchall()
    if res:
        winpycolor.colorprint(word + ' 在数据库中已存在，不需要添加', winpycolor.FOREGROUND_RED)
        sys.exit()
    ...
    try:
        ...
        winpycolor.colorprint('something\'s wrong, you can\'t add the word', winpycolor.FOREGROUND_RED)
        print(e)
    else:
        conn.commit()
        winpycolor.colorprint('%s has been inserted into database' % word, winpycolor.FOREGROUND_GREEN)
    finally:
        curs.close()
        conn.close()
...
def delete_word(word):
    ...
    if res:
        try:
            ...
            winpycolor.colorprint('%s has been deleted from database' % word, winpycolor.FOREGROUND_GREEN)
            conn.commit()
        finally:
            curs.close()
            conn.close()
    else:
        winpycolor.colorprint('%s not exists in the database' % word, winpycolor.FOREGROUND_RED)
...
def set_priority(word, pr):
    ...
    if res:
        try:
            ...
            winpycolor.colorprint('something\'s wrong, you can\'t reset priority', winpycolor.FOREGROUND_RED)
            print(e)
        else:
            winpycolor.colorprint('the priority of  %s has been reset to  %s' % (word, pr), winpycolor.FOREGROUND_GREEN)
            conn.commit()
        finally:
            curs.close()
            conn.close()
...
def list_letter(aset, vb=False, output=False):
    ...
    try:
        ...
        winpycolor.colorprint('something\'s wrong, catlog is from A to Z', winpycolor.FOREGROUND_RED)
        print(e)
    else:
        if not output:
            winpycolor.colorprint(format(aset, '-^40s'), winpycolor.FOREGROUND_GREEN)
        else:
        ...
        for line in curs.fetchall():
            ...
            print('\n' + '=' * 40 + '\n')
            if not output:
                winpycolor.colorprint('★ ' * pr, winpycolor.FOREGROUND_RED)
                winpycolor.colorprint('☆ ' * (5 - pr), winpycolor.FOREGROUND_YELLOW)
                colorful_print(expl)
            else:
                print('★ ' * pr + '☆ ' * (5 - pr))
                normal_print(expl)
...
def list_priority(pr, vb=False, output=False):
    ...
    try:
        ...
        winpycolor.colorprint('something\'s wrong, priority must be 1-5', winpycolor.FOREGROUND_RED)
        print(e)
    else:
        for line in curs.fetchall():
            expl = line[0]
...
def list_latest(limit, vb=False, output=False):
    ...
    try:
        ...
        print(e)
        winpycolor.colorprint('something\'s wrong, please set the limit', winpycolor.FOREGROUND_RED)
    else:
        for line in curs.fetchall():
            expl = line[0]
            pr = line[1]
...
def main():
    ...
    if args.add:
        ...
        if not number.isdigit():
            winpycolor.colorprint('you forget to set the number', winpycolor.FOREGROUND_RED)
            sys.exit()
        priority = int(number)
        if args.word:
            set_priority(' '.join(args.word), priority)
        else:
            winpycolor.colorprint('please set the priority', winpycolor.FOREGROUND_RED)
    elif args.letter:
        ...
        if input_file_path.endswith('.txt'):
            ...
            winpycolor.colorprint('please use a correct path of text file', winpycolor.FOREGROUND_RED)
    elif args.count:
